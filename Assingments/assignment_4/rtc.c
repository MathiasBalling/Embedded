...
void rtc_task(INT8U task_no) // this task controls the clock
{
  ...
  case RTC_RUNNING:                // if we are in the running state
    if (get_msg_event(SEB_TO_RTC)) // time out                  // if the 1s
                                   // timer has run out
    {
      ...
      if (sec >= 60) // if it is now 60 we do the same for minute counter
      {
        ...
        {
          hour =
              get_msg_state(SSM_RTC_HOUR); // read the current value for hours
          hour++;                          // increment this value
          if (hour >= 24)                  // if it is now 24
            hour = 0;                      // reset it to 0
          put_msg_state(SSM_RTC_HOUR,
                        hour); // send a message with the hour value
...
void display_rtc_task(INT8U task_no) // this task displays the clock
{
  ...
  if (wait(SEM_RTC_UPDATED)) // wait for the signal from the rtc_task that the
                             // clock has been updated
  {
    if (wait(MUTEX_LCD_DISPLAY)) // wait if the display is already being used
    {
      ...
      {
        sec = get_msg_state(
            SSM_RTC_SEC); // read the values for seconds, minutes and hours
        min = get_msg_state(SSM_RTC_MIN);
        hour = get_msg_state(SSM_RTC_HOUR);

        move_LCD(4, 0); // move the cursor
        wr_ch_LCD(
            (INT8U)(hour / 10 + '0')); // write the first digit of the hour
        wr_ch_LCD(
            (INT8U)(hour % 10 + '0')); // write the second digit of the hour
        if (sec & 0x01) // if the value for seconds ends in a 1 (so every other
                        // second)
          wr_ch_LCD(':'); // display a colon
        else
          wr_ch_LCD(
              ' '); // if the value for seconds ends in a 0, write a blank space
        ...
        signal(MUTEX_SYSTEM_RTC); // signal the to mutexes to say we are not
                                  // using the display anymore
      }
...
void adjust_rtc_task(INT8U task_no) {
  ...
  switch (state) {
  case ARTC_IDLE:
    if (get_msg_event(SEB_BUTTON_EVENT) ==
        BE_LONG_PUSH) // if a long push is registered
    {
      if (wait(MUTEX_LCD_DISPLAY)) // wait for the display mutex
      {
        if (wait(MUTEX_SYSTEM_RTC)) // wait for the rtc mutex
        {
          ...
          wr_ch_LCD(
              (INT8U)(min % 10 + '0')); // write the second digit of the minutes
        }
  ...
  case ARTC_ADJUST:
    ...
    case BE_SINGLE_PUSH: // if it is a single push
      ...
      wr_ch_LCD((INT8U)(min / 10 +
                        '0')); // write the two digits of the minutes counter
      wr_ch_LCD((INT8U)(min % 10 + '0'));
      break;
    case BE_LONG_PUSH: // if a long push is registered
      put_msg_state(SSM_RTC_MIN,
                    min);        // send the chosen minute value as a message
      move_LCD(0, 0);            // move the cursor back
      wr_ch_LCD(0xFF);           // clear the display
      signal(MUTEX_LCD_DISPLAY); // signal to say we are not using the display
                                 // anymore
